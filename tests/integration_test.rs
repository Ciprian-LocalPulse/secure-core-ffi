...
fn roundtrip_encrypt_decrypt_returns_same_message() {
    let key = [0x42u8; 32];
    let ctx = unsafe { init_security_context(key.as_ptr(), key.len()) };
    ...
    let enc_ptr = unsafe { encrypt_payload(ctx, message.as_ptr(), message.len(), &mut enc_len) };
    assert!(!enc_ptr.is_null());
    assert!(
        enc_len > message.len(),
        "the encrypted payload must include nonce + tag"
    );

    let mut dec_len: usize = 0;
    let dec_ptr = unsafe { decrypt_payload(ctx, enc_ptr, enc_len, &mut dec_len) };
...
fn invalid_key_returns_null_context() {
    ...
    let ctx = unsafe { init_security_context(short_key.as_ptr(), short_key.len()) };
    assert!(ctx.is_null());
}

#[test]
fn null_output_length_is_rejected() {
    let key = [0x11u8; 32];
    let ctx = unsafe { init_security_context(key.as_ptr(), key.len()) };
    assert!(!ctx.is_null());
    ...
    assert!(
        unsafe { encrypt_payload(ctx, message.as_ptr(), message.len(), std::ptr::null_mut()) }
            .is_null()
    );
    assert!(
        unsafe { decrypt_payload(ctx, message.as_ptr(), message.len(), std::ptr::null_mut()) }
            .is_null()
    );

    unsafe { free_security_context(ctx) };
}

#[test]
fn invalid_key_length_is_rejected_before_reading_key() {
    let key = [0u8; 1];
    assert!(unsafe { init_security_context(key.as_ptr(), usize::MAX) }.is_null());
}
...
fn decryption_of_corrupted_data_fails_gracefully() {
    ...
    let ptr = unsafe {
        decrypt_payload(
            ctx,
            corrupted_payload.as_ptr(),
            corrupted_payload.len(),
            &mut out_len,
        )
    };
    assert!(
        ptr.is_null(),
        "decryption must fail gracefully for corrupted data"
    );
